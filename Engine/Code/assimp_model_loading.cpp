...
void ModelLoader::ProcessAssimpMaterial(App* app, aiMaterial *material, Material& myMaterial, String directory)
{
    ...
    myMaterial.SetAlbedo(vec3(diffuseColor.r, diffuseColor.g, diffuseColor.b));
    myMaterial.SetEmissive(vec3(emissiveColor.r, emissiveColor.g, emissiveColor.b));
    ...
    if (material->GetTextureCount(aiTextureType_DIFFUSE) > 0)
    {
        ...
        myMaterial.SetAlbedoTextureIdx(LoadTexture2D(app, filepath.str));
    }
    if (material->GetTextureCount(aiTextureType_EMISSIVE) > 0)
    {
        ...
        String filename = MakeString(aiFilename.C_Str());
        String filepath = MakePath(directory, filename);
        myMaterial.SetEmissiveTextureIdx(LoadTexture2D(app, filepath.str));
    }
    if (material->GetTextureCount(aiTextureType_SPECULAR) > 0)
    {
        ...
        String filename = MakeString(aiFilename.C_Str());
        String filepath = MakePath(directory, filename);
        myMaterial.SetSpecularTextureIdx(LoadTexture2D(app, filepath.str));
    }
    if (material->GetTextureCount(aiTextureType_NORMALS) > 0)
    {
        ...
        String filename = MakeString(aiFilename.C_Str());
        String filepath = MakePath(directory, filename);
        myMaterial.SetNormalsTextureIdx(LoadTexture2D(app, filepath.str));
    }
    if (material->GetTextureCount(aiTextureType_HEIGHT) > 0)
    {
        ...
        String filename = MakeString(aiFilename.C_Str());
        String filepath = MakePath(directory, filename);
        myMaterial.SetBumpTextureIdx(LoadTexture2D(app, filepath.str));
    }
...
void ModelLoader::ProcessAssimpNode(const aiScene* scene, aiNode* node, Mesh* myMesh, u32 baseMeshMaterialIndex, std::vector<u32>& submeshMaterialIndices)
{
    ...
    for(unsigned int i = 0; i < node->mNumMeshes; i++)
    {
        ...
        ModelLoader::ProcessAssimpMesh(scene, mesh, myMesh, baseMeshMaterialIndex, submeshMaterialIndices);
    }

    // then do the same for each of its children
    for(unsigned int i = 0; i < node->mNumChildren; i++)
    {
        ModelLoader::ProcessAssimpNode(scene, node->mChildren[i], myMesh, baseMeshMaterialIndex, submeshMaterialIndices);
    }
}
